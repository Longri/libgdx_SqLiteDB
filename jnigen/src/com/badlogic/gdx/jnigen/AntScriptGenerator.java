...
public class AntScriptGenerator {
    ...
    public void generate(BuildConfig config, Array<BuildTarget> targets) {
        ...
        String template = new FileDescriptor("../jnigen/src/com/badlogic/gdx/jnigen/resources/scripts/build.xml.template", FileType.Absolute)
                .readString();
...
//        }
        ...
        //changed for local handling

        final String pack = "../jnigen/src/com/badlogic/gdx/jnigen/resources/headers";
        String files[] = {"classfile_constants.h", "jawt.h", "jdwpTransport.h", "jni.h", "linux/jawt_md.h", "linux/jni_md.h",
                "mac/jni_md.h", "win32/jawt_md.h", "win32/jni_md.h"};

        for (String file : files) {
            new FileDescriptor(pack, FileType.Absolute).child(file).copyTo(
                    new FileDescriptor(jniDir).child("jni-headers").child(file));
        }
    ...
    private String generateBuildTargetTemplate(BuildConfig config, BuildTarget target) {
        ...
        if (target.os == TargetOs.Android) {
            ...
            String template = new FileDescriptor("../jnigen/src/com/badlogic/gdx/jnigen/resources/scripts/build-android.xml.template",
        ...
        if (target.os == TargetOs.IOS) {
            template = new FileDescriptor("../jnigen/src/com/badlogic/gdx/jnigen/resources/scripts/build-ios.xml.template", FileType.Absolute)
                    .readString();
        } else {
            template = new FileDescriptor("../jnigen/src/com/badlogic/gdx/jnigen/resources/scripts/build-target.xml.template", FileType.Absolute)
                    .readString();
        }
